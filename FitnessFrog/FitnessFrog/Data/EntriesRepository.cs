...
namespace FitnessFrog.Data
{
    ...
    public class EntriesRepository
    {
        ...
        {
            Entry entry = Data.Entries.SingleOrDefault(e => e.Id == id);

            // Ensure that the activity property is not null.
            if (entry != null && entry.Activity == null)
            {
        ...
        public void UpdateEntry(Entry entry)
        {
            ...
            if (entryIndex == -1)
            {
                throw new Exception($"Unable to find an entry with an ID of {entry.Id}");
            }
        ...
        public void DeleteEntry(int id)
        {
            ...
            if (entryIndex == -1)
            {
                throw new Exception($"Unable to find an entry with an ID of {id}");
            }
